...
if [ -f "$config_file" ]; then
    ...
    curl -k --cert assets/ssl/admin.pem --key assets/ssl/admin-key.pem -XPATCH https://0.0.0.0:9200/_plugins/_security/api/internalusers -H 'Content-Type: application/json' -d'
    [
      {
    ...
    curl -k --cert assets/ssl/admin.pem --key assets/ssl/admin-key.pem -XPUT "https://0.0.0.0:9200/_plugins/_security/api/roles/dashboards" -H 'Content-Type: application/json' -d'
    {
      "cluster_permissions": [
        "cluster_all",
        "indices_monitor"
    ...
    curl -k --cert assets/ssl/admin.pem --key assets/ssl/admin-key.pem -XPUT "https://0.0.0.0:9200/_plugins/_security/api/rolesmapping/kibana_server" -H 'Content-Type: application/json' -d'
    {
    ...
    curl -k --cert assets/ssl/admin.pem --key assets/ssl/admin-key.pem -XPUT "https://0.0.0.0:9200/_plugins/_security/api/rolesmapping/readall" -H 'Content-Type: application/json' -d'
    {
